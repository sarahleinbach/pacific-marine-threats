...
with open('threats.csv', mode='r') as csv_file:
...
with open("threat_taxa.txt", mode='w+') as outfile:
    outfile.write("Threats:\n")
    for key in threats:
        outfile.write(threat_dict[key] + ': (' + str(len(threats[key])) + ' species) ')
        outfile.write(', '.join(threats[key]) + '\n')

taxa_list = ['ARTHROPODA','ACTINOPTERYGII','MAMMALIA','CHONDRICHTHYES','REPTILIA','MYXINI','CNIDARIA','MOLLUSCA','HOLOTHUROIDEA']
taxa = c.defaultdict(set)

with open('taxonomy.csv', mode='r') as csv_file:
    csv_reader = csv.DictReader(csv_file)
    line_count = 0
    for row in csv_reader:
        x = row['phylumName'] in taxa_list
        y = row['className'] in taxa_list
        if x:
            taxa[row['phylumName']].add(row['scientificName'])
        elif y:
            taxa[row['className']].add(row['scientificName'])

with open("taxa_groups.txt", mode='w+') as outfile:
    outfile.write("Taxa:\n")
    for key in taxa:
        outfile.write(key + ': (' + str(len(taxa[key])) + ' species) ')
        outfile.write(', '.join(taxa[key]) + '\n')
